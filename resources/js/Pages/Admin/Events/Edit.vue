...
const form = useForm({
    ...
    speaker_name: props.event.speaker_name ?? '',
    location: props.event.location ?? '',
    timezone: props.event.timezone || props.defaultTimezone,
    all_day: props.event.all_day,
    start_date: props.event.start_date ?? '',
    end_date: props.event.end_date ?? '',
    starts_at: props.event.starts_at_local ?? '',
    ends_at: props.event.ends_at_local ?? '',
    recurrence_frequency: props.event.recurrence_frequency ?? 'none',
    recurrence_on: props.event.recurrence_on ?? '',
    exclusion_dates: [...(props.event.exclusion_dates ?? [])],
    featured_image: null,
...
const { displaySrc: previewImageSrc } = useFeaturedPreview(form, () => props.event.featured_image_url);
...
const submit = () => {
    form.put(route('admin.events.update', props.event.id), { forceFormData: true });
};
</script>
    ...
    <DashboardLayout title="Edit event">
        <template #header>
            <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                ...
                <Link
                    :href="route('admin.events.preview', event.id)"
            ...
            <div class="grid gap-8 lg:grid-cols-2 lg:items-start">
                <div class="min-w-0">
                    ...
                    <div>
                        ...
                        <div v-if="event.featured_image_url && !form.remove_featured_image" class="mt-2">
                            <img
                                :src="event.featured_image_url"
                                alt=""
                                class="h-32 w-auto max-w-full rounded-md border border-border object-cover"
                            >
                        </div>
                        <label v-if="event.featured_image_url" class="mt-2 flex items-center gap-2 text-sm text-ink">
                            <input
                        ...
                        <input
                            type="file"
                            accept="image/*"
                            class="mt-2 block w-full text-sm text-ink-muted file:mr-4 file:rounded-md file:border-0 file:bg-accent file:px-3 file:py-1.5 file:text-sm file:font-medium file:text-accent-fg hover:file:bg-accent-hover"
